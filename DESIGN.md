...
- **Song page, the song leads:** desktop is lyrics left, a sticky sidebar right (recording, the Tom and C D E / Dó Ré Mi bar, tags), prev/next under the lyrics. On phones (≤768px) the order is title, controls, the Tom bar, lyrics, then the recording and tags, then prev/next, so the first lyric is on the first screen (about 490px down at 390 × 844). The Tom bar renders once per placement and CSS shows one; below 360px it wraps to two rows.
